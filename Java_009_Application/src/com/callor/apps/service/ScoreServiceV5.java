...
public class ScoreServiceV5 {
	...
	/*
	 * 현재는 학생수만 큼 모든 과목의 점수 입력을 끝내야만 다른 업무로 진행이 된다 몇몇학생, 일부 과목 점수만 이력마치고 잠시 업무를 멈추고
	 * 싶을 떄 현재 코드는 불가능핟아 =>일부만 입력하고 업무를 중단하는 기능을 넣자
	 */
	public void makeScores() {
		...
		for (int i = 0; i < intKor.length; i++) {
			...
			for (int j = 1; j < subject.length; j++) {
				...
				/*
				 * 입력업무중에 -1을 입력했을 때는 입력업무를 중단하고 메인메서드로 돌아가고 싶다 이때 브레이크를 사용하면 포문이 겹쳐있기 때문에 원하는
				 * 대로 코드가 작동되지 않는다 이럴 경우 리턴타입이 보이드 이지만 리턴명령을 실행하여 메서드 코드를 중단할 수 있다
				 */
				if (score[j] == -1) {
					return;
				}
