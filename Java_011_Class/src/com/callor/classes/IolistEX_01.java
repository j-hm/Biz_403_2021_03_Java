...
public class IolistEX_01 {
	 ...
	 *  2. 컴파일 : IolictEX_01.java파일을 번역하여
	 *  			컴퓨터가 알수 있는 부호들(코드)로 변환하는 작업
	 *  			=> javac IolictEX_01.java
	 *  3. 실행 : 컴파일 되어 생성된 IolictEX_01.class 파일을
	 *  		  컴퓨터 메모리에 load하고 프로젝트를 실행
	 *  		  => java IolictEX_01
	 *  4. 가상머신 : 자바코드를 번역한 *.class파일을 열어서
	 *  			  코드를 한개씩 운영체제, CPU에게 보내서
	 *  			  결과를 얻을 수 있게 하는 어플리케이션.
	 *  			  운영체제에 관계없이 코드를 실행하여 
	 *  			  같은 결과를 내도록 하는 역할을 수행한다
	 *  5. 진입점 method : static으로 작성된 main()method
	 *  				   가상머신이 java로 컴파일된 코드파일에서
	 *  				   제일 먼저 찾는 method.
	 *  				   java IolictEX_01라고 실행을 지시하면
	 *  				   Iolist_01.main("qkrtjdghk")를 호출하여 코드를 실행하기 시작한다
	 */
	public static void main(String[] args) {
		
		/*
		 * VO클래스는 데이터를 저장하여 연산이 필요한 곳으로 이동, 복사 등을 할 때
		 * 사용하는 도구, 바구니 역할을 한다
		 * 
		 * VO클래스에 데이터를 저장한다
		 ...
		 *  3. 지금 만든 IolistVO클래스에 멤버변수는 모두 프라이빗이기 때문에
	...
	public static void printIolist(IolistVO iolistVO) {
		System.out.println("=====================================================");
		System.out.println("상품명\t일자\t\t거래처\t구분\t단가\t수량");
		System.out.println("-----------------------------------------------------");
