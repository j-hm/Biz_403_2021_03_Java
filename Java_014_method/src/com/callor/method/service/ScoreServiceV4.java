package com.callor.method.service;

public class ScoreServiceV4 {

	/*
	 * 과목의 이름이 저장될 배열을 클래스 영역에 선언 
	 * 과목점수 입력, 성적리스트 출력 등에서 사용하기 때문에 
	 * 멤버변수 영역, 클래스 영역에 선언
	 */
	protected String[] subject;
	protected InputServiceV2 inScore;

	/*
	 * 생성자 메서드 ScoreServiceV4클래스를 사용하여 객체를 생성할 때 호출할 메서드
	 */
	public ScoreServiceV4() {
		// 생성한다 :
		// 변수에 값을 저장, 객체를 초기화(new 생성자() 호출)
		// 생성자에 작성되는 코드
		// 멤버변수 영역에 선언된 변수, 객체를 생성하는 코드
		subject = new String[] { "국어", "영어", "수학" };
		inScore = new InputServiceV2();
	}

	// 키보드를 사용하여 3과목의 점수를 입력받는데
	// InputServiceV2에 선언된 메서드를 사용할 예정이므로
	// 여기에는 키보드등의 클래스, 객체를 사용할 필요가 없다
	public void inputScore() {

		// 과목명을 배열로 선언한 이유는
		// 점수를 입력받는 메서드 호출코드를 한번만 작성하고
		// for()반복문을 사용하여 처리할 수 있다

		// 과목의 점수를 저장할 score배열을 생성
		int[] scores = new int[subject.length];
		for (int i = 0; i < subject.length; i++) {
			Integer score = inScore.inputValue(subject[i], 0, 100);
			if (score == null) {
				System.out.println("업무 종료");
				/*
				 * break ; break를 실행하면 for()반복문만 중단하고 
				 * ; 반복문 이후의 코드를 실행한다 
				 * ; 반복문 이후에 다른 연산을 수행할 필요가 있을 때
				 * 
				 * return
				 *  ; return을 실행하면 현재 inputScore()메서드를 무조건 종료해버린다
				 *  ; inputScore()메서드를 호출했던 코드로 되돌아간다
				 */
				return;
			} // return받은 점수를 scores배열의 i번째 위치에 저장하라
			scores[i] = score;
		} // for end
		
		//scores배열에 담긴 총점계산
		int sum = 0;
		for (int i = 0; i < scores.length; i++) {
			sum += scores[i];
		}
		//과목의 개수만큼으로 평균을 계산한다
		float avg = (float) sum / scores.length;

		System.out.println("=".repeat(50));
		System.out.printf("총점 : %d\t", sum);
		System.out.printf("평균 : %3.2f", avg);

	}

}
